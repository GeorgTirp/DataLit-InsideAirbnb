...
def download_image(image_url, image_size=(256, 256)):
    """Download and process a single image."""
    if isinstance(image_url, float):  # NaN or invalid URL
        return Image.new("RGB", image_size)
    
    try:
        response = requests.get(image_url, timeout=5)  # Add timeout for faster failure
...
class InsideAirbnbDataset:
    ...
    def _integrate_reviews_into_listings(self):
        ...
        for city in self.cities:
            ...
            print(city_listings.columns)
            print(city_reviews.columns)
            ...
            for image_url_col_name in image_url_col_names:
                ...
                with ThreadPoolExecutor(max_workers=10) as executor:
                    ...
                    for i, image_url in enumerate(tqdm(image_url_col)):
                        ...
                        future = executor.submit(download_image, image_url, image_size)
                ...
                for i, image in enumerate(image_list):
                    print(f"Saving image {i/len(image_list)}")
                    ...
                    if type(image_url) is float:
                        ...
                        if response.status_code == 200:
                            try:
                                ...
                                print(f"An error occurred: {e}")  # You could log the error if necessary
                                no_access_indices.append(i)
                                image_list.append(Image.new("RGB", image_size))
                        else:
                            no_access_indices.append(i)
...
if __name__ == "__main__":
    if not DEBUG_MODE:
        for folder in os.listdir('/home/sn/pCloudDrive/AirBnB_Daten/Raw_data'):
            folder_path = os.path.join('/home/sn/pCloudDrive/AirBnB_Daten/Raw_data', folder)
            if os.path.isdir(folder_path):
                print(f"Processing folder: {folder}")
                print((f'Filename: {folder}_listings.csv'))
                for city in os.listdir(folder_path):
                        print(f"Processing folder: {city}")
                        listings_file = os.path.join(folder_path, city, 'listings.csv')
                        reviews_file = os.path.join(folder_path, city, 'reviews.csv')
                        calendar_file = os.path.join(folder_path, city, 'neighbourhoods.csv')

                        if os.path.exists(listings_file):
                            os.remove(listings_file)
                            print(f"Deleted file: {listings_file}")

                        if os.path.exists(reviews_file):
                            os.remove(reviews_file)
                            print(f"Deleted file: {reviews_file}")

                        if os.path.exists(calendar_file):
                            os.remove(calendar_file)
                            print(f"Deleted file: {calendar_file}")

                        listings_gz_file = os.path.join(folder_path, city, 'listings.csv.gz')
                        if os.path.exists(listings_gz_file):
                            with gzip.open(listings_gz_file, 'rb') as f_in:
                                with open(listings_file, 'wb') as f_out:
                                    shutil.copyfileobj(f_in, f_out)
                            print(f"Unzipped file: {listings_gz_file}")

                        listings_gz_file = os.path.join(folder_path, city, 'reviews.csv.gz')
                        if os.path.exists(listings_gz_file):
                            with gzip.open(listings_gz_file, 'rb') as f_in:
                        ...
                        listings_gz_file = os.path.join(folder_path, city, 'neighbourhoods.csv.gz')
                        if os.path.exists(listings_gz_file):
                            with gzip.open(listings_gz_file, 'rb') as f_in:
                                with open(calendar_file, 'wb') as f_out:
                                    shutil.copyfileobj(f_in, f_out)
                            print(f"Unzipped file: {listings_gz_file}")

                data_set = InsideAirbnbDataset(raw_data_dir=folder_path, preprocessed_data_dir=f'/home/sn/pCloudDrive/AirBnB_Daten/Preprocessed_data/{folder}_preprocessed')
                print('All listings: ', data_set.all_cities_listings.columns)
                data_set.save_all_cities_listings_to_file(file_name=f"city_listings.csv", saving_dir=f'/home/sn/pCloudDrive/AirBnB_Daten/Preprocessed_data/{folder}_preprocessed')
                #data_set.add_nlp_embedding(nlp_col_names=['name'])
                data_set.download_images_and_save(saving_dir=f'/home/sn/pCloudDrive/AirBnB_Daten/Preprocessed_data/filtered_dataset_images')
                #data_set.add_image_embedding(read_from_dir=True, read_dir=f'/home/sn/pCloudDrive/AirBnB_Daten/Preprocessed_data/filtered_dataset_images')
        
        #data_set = InsideAirbnbDataset(RAW_DATA_DIR='/home/sn/pCloudDrive/AirBnB_Daten/argentina', SAVING_DIR='/home/sn/pCloudDrive/AirBnB_Daten/Preprocessed_data')
    else:
        main()
